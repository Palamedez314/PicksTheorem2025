...
\begin{figure}[ht]
  \begin{tikzpicture}[x=10mm, y=10mm, inner sep=1pt, font=\footnotesize]
    ...
    node[pos=0.5, sloped, below]{$-1/4$};
    \draw[thick, black!60, -latex'] (-3.0,+1.2) -- (-3.2,-1.0);
    \draw[thick, black!60, -latex'] (-3.8,-1.0) -- (-3.9,+1.2);
    \draw[thick, black!60, -latex'] (+3.0,-1.2) -- (+3.2,+1.0);
    \draw[thick, black!60, -latex'] (+3.8,+1.0) -- (+3.9,-1.2);
...
\begin{figure}[ht]
  \begin{tikzpicture}[x=5mm, y=5mm, font=\footnotesize\mathstrut]
    ...
    \begin{scope}[shift={(9,0)}]
      ...
      \draw (5.5,3.5) node[right, align=center]{
        discrete homotopy \\ from the hypotenuse \\ to the catheti};
    \end{scope}
  \end{tikzpicture}
  \caption{Proof of Hopf's theorem}
...
(He then showed that it suffices to require this condition piecewise.)
He proved his theorem by an ingenious homotopy argument.
...
\begin{proof}[Hopf's homotopy proof]
  ...
  und $x_{n-2} < x_{n-1} < x_0 < x_1$.
  (This is shown on the left of \autoref{fig:HopfTheorem}.)
  
  For $0 \le i < j \le n-1$ we consider the secant $s_{i,j} := p_j - p_i$.
  From $s_{0,1}$ via $s_{0,j}$ to $s_{0,n-1}$ we perform a first half turn,
  then via $s_{i,n-1}$ to $s_{n-2,n-1}$ we perform a second half turn.
  ...
  The left hand side of \eqref{eq:Hopf} is the total tangent turning number.
  We replace each summand using (1) and then add all terms (2).
  Most of the summands cancel pairwise.
  The remaining terms yield the right hand side of \eqref{eq:Hopf}.
