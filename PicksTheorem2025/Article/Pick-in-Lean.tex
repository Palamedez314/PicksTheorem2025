...
% tex.stackexchange.com/questions/26637/how-do-you-get-mathbb1
...
\usepackage[pdftex,
            ...
            pdftitle={Formalizing Pick's Theorem, efficiently}, % in Lean},
            pdfstartview=FitH,
            colorlinks=true,
            anchorcolor=black, 
            linkcolor=black, 
...
\date{Burg Liebenzell, first draft September 2024, this version compiled \today}
...
\begin{remark}
  ...
  Otherwise, if $\Area(P) < 0$, we reverse $P = (p_0,p_1,\ldots,p_n)$
...
Usually this case is ignored, forbidden, or arbitrated.
...
\begin{remark}
  ...
  This groundwork being provided, we can now focus on proving Pick's lemma.
\end{remark}
...
\begin{remark}
  In this algebraic form, the proof is readily implemented in a proof assistant.
  For a proof in Lean see \url{https://github.com/Palamedez314/PicksTheorem2025}.
  This completes the promised formalization.
\end{remark}
...
First, we need Jordan's decomposition
to formulate Pick's theorem precisely.
Second, we add Hopf's umlaufsatz to interpret
the counting result in a more geometric and user-friendly fashion.
...
\begin{definition}[angle measure\label{def:AngleMeasure}]
  ...
  \item
    Normalization: $\mu(e_1,e_2) = \sfrac{1}{4}$.
  \end{enumerate}
\end{definition}
...
\begin{remark}
  ...
  now acquire an algebraic flavour over $\K$:
  ...
  (3) The extension to boundary points $q \in C$ % is usually omitted.
...
\begin{remark}
  ...
  in any triangle, the interior angles add up
  to $\beta_1 + \beta_2 + \beta_3 = 1/2$,
  corresponding to $\pi/2$ or $180^\circ$.
  ...
  \begin{tikzpicture}[x=5mm, y=5mm, font=\footnotesize\mathstrut]
    ...
    \begin{scope}[shift={(9,0)}]
      ...
      \foreach \i [
        ...
        \foreach \j in {\ii,...,5}{
          \draw[draw=black, fill=teal!10] (\i,\j) rectangle (\i+1,\j+1);
        }
      }
      \foreach \i in {0,...,4}{
...
\section{A “water proof” plausibility argument}
...
% See Harrison on the tension between intuition and rigour.
...
% First, the point count.  Second, the umlaufsatz.
